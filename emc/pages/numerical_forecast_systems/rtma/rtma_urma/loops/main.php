...
<div class="mapbox">
<!--<img id="mapBoxImg" src="https://www.emc.ncep.noaa.gov/mmb/jcarley/rtma_urma/prod/RTMA/13/pan_t2m_CONUS_RTMA.gif" name="animation" alt="Loading..." ondblclick="document.location.href = this.src" />
-->
<img id="mapBoxImg" src="" name="animation" alt="Loading..." ondblclick="document.location.href = this.src" />
...
$result = $result."/RTMA_CONUS_t2m.php";
#$fileName = "../../../../../../mmb/rtma/prod/RTMA/index.html";
#$str = "<base";
#$result = "";
...
//baseValUrl = baseValUrl.slice(baseValUrl.indexOf('RTMA')+5, baseValUrl.length);
//console.log('Value is the following: ' + baseValUrl);
var titles = ['gust10m', 't2m', 'vis', 'td2m', 'spd10m', 'vec10m', 'sfcp', 'tcamt', 'ceiling', 'spfh'];
var htmlAnchor = '<a class="dropdown-item lnkm1"  href="//www.emc.ncep.noaa.gov/mmb/rtma/prod/RTMA/';

/***************************************************************************
* When the Page finish loading get the current image of the current UTC time
***************************************************************************/
var dateNow = new Date;
var loadHref = 'https://www.emc.ncep.noaa.gov/mmb/rtma/prod/RTMA/'+ baseValUrl.split('/')[0]  +'/pan_t2m_CONUS_RTMA.gif';
//var loadHref = 'https://www.emc.ncep.noaa.gov/mmb/jcarley/rtma_urma/prod/RTMA/'+ dateNow.getUTCHours()  +'/pan_t2m_CONUS_RTMA.gif';
 ...
 var linkEvent = document.getElementsByClassName("lnkm1");
  ...
  for (var i=0; i < linkEvent.length;i++){
      ...
      linkEvent[i].addEventListener('click', function(e){
        ...
        if (text.includes('UTC')){
         var cycle = text.substring(0,2);
         sessionStorage.setItem("cycle", cycle);
         console.log("Inner Text"+text);
        }else{
         sessionStorage.setItem("region", text);
        }
...
function loadChooseCycleDrpdwn(id){
  //document.getElementById(id).innerHTML = htmlAnchor + '/index.html"> Most recent </a>';
  document.getElementById(id).innerHTML = '<a class="dropdown-item" href="main.php"> Most recent </a>';
  for(var i = 0; i < 24; i++){
      var url = "https://www.emc.ncep.noaa.gov/mmb/rtma/prod/RTMA/";
      url = url+pad(i)+'/pan_t2m_CONUS_RTMA.gif';
     ...
     if (a.length > 0){
      ...
      //console.log(now.getUTCHours());
       for (var j=0; j < titles.length; j++){
          ...
          var href_lnk = htmlAnchor + baseValUrl.split('/')[0] +'/pan_'+titles[j]+'_'+fileTitle+'_RTMA.gif">'+titles[j]+'</a>';
          document.getElementById(id).innerHTML += href_lnk;
       }
      displayImg(); 
     }
...
var base_url = 'https://www.emc.ncep.noaa.gov/mmb/rtma/prod/RTMA/'+ baseValUrl.split('/')[0];
...
window.temp_list[0]= base_url+'/pan_t2m_CONUS_RTMA.gif';
window.temp_list[1]= base_url+'/t2m_CONUS_RTMA_ANL.gif';
window.temp_list[2]= base_url+'/t2m_CONUS_RTMA_GES.gif';
window.temp_list[3]= base_url+'/inc_t2m_CONUS_RTMA.gif';
window.temp_list[4]= base_url+'/obs_t2m_CONUS_RTMA.gif';
window.temp_list[5]= base_url+'/usedobs_t2m_CONUS_RTMA.gif';
initialize_looper();
...
var linkEvent = document.getElementsByClassName("lnkm1");
  ...
  for (var i=0; i < linkEvent.length;i++){
      ...
      linkEvent[i].addEventListener('click', function(e){
        e.preventDefault();
        console.log(document.animation.src);
       //console.log(this.getAttribute('href'));
       //document.animation.src = this.getAttribute('href');
       //console.log(this);
       for (var j=0; j < navTitles.length; j++){
        if (this.getAttribute('href').indexOf(navTitles[j]) >= 0){
          ...
          window.temp_list[3]= this.getAttribute('href').replace('pan','inc'); //base_url+'/inc_t2m_CONUS_RTMA.gif';
          window.temp_list[4]= this.getAttribute('href').replace('pan','obs');//base_url+'/obs_t2m_CONUS_RTMA.gif';
          window.temp_list[5]= this.getAttribute('href').replace('pan','usedobs');//base_url+'/usedobs_t2m_CONUS_RTMA.gif';
